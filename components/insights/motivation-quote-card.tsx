// Motivational quote — Quotable public API (no key). Flexbox layout: column stack + header row + footer row.
    ...
    <View style={[styles.card, { backgroundColor: surfaceMuted, borderColor }]} accessibilityLabel="Motivational quote card">
      ...
      <View style={styles.headerRow}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [styles.refreshPressable, { opacity: pressed ? 0.75 : loading ? 0.55 : 1 }]}>
          <ThemedText style={[styles.refreshLabel, { color: tint }]}>{refreshLabel}</ThemedText>
        </Pressable>
      </View>

      <ThemedText style={[styles.kicker, { color: mutedColor }]}>
        Short quote from Quotable (motivational tag) — no sign-up, no API key.
      </ThemedText>
